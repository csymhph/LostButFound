import h5py
import random
from utils.data.transforms import DataTransform, KspaceTransform
...
def create_kspace_loaders(data_path_1, data_path_2, args, shuffle=True, isforward=False):
    if isforward == False:
        max_key_ = args.max_key
        target_key_ = args.target_key
    else:
        max_key_ = -1
        target_key_ = -1
    data_storage = KSliceData(
        root1=data_path_1,
        root2=data_path_2,
        # initialize isforward, max_key_
        transform=KspaceTransform(isforward, max_key_),
        input_key=args.input_key,
        target_key=target_key_,
        forward = isforward
    )
